...
**Optional** Path to a file that holds the Flutter version to use, such as `pubspec.yaml` or `.fvmrc`, resolved from the repository root rather than `working_directory`. This lets you keep a single source of truth for the Flutter version. When set, the workflow uses Flutter instead of Dart to install dependencies.
