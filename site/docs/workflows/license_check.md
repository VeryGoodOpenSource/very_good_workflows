...
**Optional** The path to a file containing the Flutter version to use (e.g. `pubspec.yaml` or `.fvmrc`), resolved from the repository root rather than `working_directory`. This lets you keep a single source of truth for the Flutter version. When set, Flutter is used instead of Dart to install dependencies.
